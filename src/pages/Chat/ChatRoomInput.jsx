...
function ChatRoomInput({ setChatMessages }) {
  ...
  const handleInputSubmit = (e) => {
    ...
    postUserMessage(
      ...
      (data) => {
        // Update user's room message
        setChatMessages((prev) => [...prev, { ...data.data, avatarImage: user.avatarImage }]);

        // Send real-time notification to the other user via socket
  ...
  const handleKeyUp = () => {
    // Only emit if typing status changes
  ...
  useEffect(() => {
    if (typingNotify) {
      ...
      setShowNotify(typing && isChatting); // Show only if typing and in the chat
    } else {
      setShowNotify(false);
    }
  }, [typingNotify, chatId]);
