...
function ChatContextProvider({ children }) {
  ...
  // Update the latest message for a contact
  ...
  // When there is a new message, update contact's latest message
  useEffect(() => {
    if (messageData) {
  ...
  // Notify the other user that this user has read the message
  const updateMessageStatusToRead = (chatId, type) => {
    // API: Update the other user's message as read
    updateReadStatus({
      method: 'PUT',
    ...
    // Socket: Inform the other user that "this user" has read the message
  ...
  const handleChatSelect = async (selected) => {
    if (selected._id !== chatId) {
      if (selected.chatType === 'room') {
        socketEmitEvent(socket).enterChatRoom({ roomId: selected._id, message: `${user.name} has joined the chat` });
      }
      if (chatInfo?.chatType === 'room') {
        socketEmitEvent(socket).leaveChatRoom({ roomId: chatId, message: `${user.name} has left the chat` });
      }
